...
extern "C" ggml_backend_t llamar_cpu_init_shim();
...
llama_context::llama_context(
    ...
    if (!hparams.vocab_only) {
        // CPU-only backend via local shim (avoids C-linkage symbol)
        backend_cpu = llamar_cpu_init_shim();
    ...
    if (!hparams.vocab_only) {
        ...
        for (auto & backend : backends) {
            ...
            ggml_backend_dev_t dev = ggml_backend_get_device(backend.get());
            auto backend_type = dev ? ggml_backend_dev_type(dev) : GGML_BACKEND_DEVICE_TYPE_CPU;
        ...
        if (pipeline_parallel) {
            for (auto & backend : backends) {
                ...
                // dev is guaranteed non-null here because non-CPU backends provide a device
