...
static bool weight_buft_supported(const llama_hparams & hparams, ggml_tensor * w, ggml_op op, ggml_backend_buffer_type_t buft, ggml_backend_dev_t dev) {
    ...
    if (dev != nullptr) {
        ...
        // CPU-only path: when no device registry is used, treat CPU as supported
        op_supported = true;
    }
...
static buft_list_t make_cpu_buft_list(const std::vector<ggml_backend_dev_t> & /*devices*/, bool /*use_extra_bufts*/) {
    ...
    // CPU-only: just use the CPU buffer type
...
bool llama_model::load_tensors(llama_model_loader & ml) {
    ...
    // CPU-only: no device registry; treat CPU as default
    ...
    auto get_layer_buft_list = [&](int il) -> llama_model::impl::layer_dev {
        ...
        LLAMA_LOG_DEBUG("load_tensors: layer %3d assigned to device %s, is_swa = %d\n", il, devices.empty() ? "CPU" : "GPU", is_swa);
    ...
    for (auto & it : ctx_map) {
        ...
        // CPU-only path: allocate from CPU buffer type
...
static bool buft_supported(ggml_backend_buffer_type_t buft, ggml_backend_dev_t dev, F & fn) {
    ...
    if (dev != nullptr) {
        ...
        // CPU-only path with no device registry: assume CPU support
        op_supported = true;
    }
